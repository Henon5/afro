...
gameSessionSchema.methods.checkWin = function(playerIndex) {
  ...
  const isValidMarked = (r, c) => (r === 2 && c === 2) ? true : this.calledNumbers.includes(cardGrid[r][c]) && markedState[r][c];
  
  for (let r = 0; r < 5; r++) if ([0,1,2,3,4].every(c => isValidMarked(r, c))) return { win: true, pattern: `row-${r}` };
  for (let c = 0; c < 5; c++) if ([0,1,2,3,4].every(r => isValidMarked(r, c))) return { win: true, pattern: `col-${['B','I','N','G','O'][c]}` };
